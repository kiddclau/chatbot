...
//	}
	private GeographyDto toGeographyDto(Geography basic) {
		return new GeographyDto(basic.getId(), basic.getPlace_name(),basic.getClimate(),basic.getPopulation(),basic.getHistory(),basic.getOther_details());
	}

	@Override
	@Transactional
	public GeographyDto readTownFromName(String place_name) {
		Optional<Geography> town = basicRepo.findPlaceByName(place_name);
		if (town.isPresent()) {
			//LOGGER.info(() -> String.format("The student with name %s is allready present",name));
			return toGeographyDto(town.get());
		} else {
			return null;
